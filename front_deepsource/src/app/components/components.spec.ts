...
// JS-0356: FormsModule eliminado (nunca usado)
...
describe('LoginComponent', () => {
  ...
  // JS-0323: reemplaza {} as any por objeto tipado
  const mockLoginResponse: LoginResponse = {
    mensaje: 'ok',
    usuario: { nombre: '', apellido: '', correo: '', nombreUsuario: '', telefono: '' }
  };
  ...
  it('submit() exitoso redirige a /admin cuando es admin', fakeAsync(() => {
    auth.login.and.returnValue(of(mockLoginResponse));
...
describe('RegisterComponent', () => {
  ...
  it('registrar() exitoso redirige a /login después de 1800ms', fakeAsync(() => {
    auth.registrar.and.returnValue(of(mockLoginResponse));
    component.paso = 2;
...
describe('HistorialComponent', () => {
  ...
  it('colorTipo() devuelve el color correcto para cada tipo', () => {
    expect(HistorialComponent.colorTipo('AUDIO')).toBe('#FF6B4A');
    expect(HistorialComponent.colorTipo('VIDEO')).toBe('#4361EE');
    expect(HistorialComponent.colorTipo('IMAGEN')).toBe('#2EC4B6');
    expect(HistorialComponent.colorTipo('OTRO')).toBe('#888');
  });

  it('iconoEstado() devuelve el ícono correcto', () => {
    expect(HistorialComponent.iconoEstado('COMPLETADO')).toBe('✓');
    expect(HistorialComponent.iconoEstado('PENDIENTE')).toBe('⏳');
    expect(HistorialComponent.iconoEstado('EN_PROCESO')).toBe('⚙️');
    expect(HistorialComponent.iconoEstado('FALLIDO')).toBe('✕');
    expect(HistorialComponent.iconoEstado('DESCONOCIDO')).toBe('?');
  });
});
...
describe('AdminDashboardComponent', () => {
  ...
  it('colorTipo() retorna el color correcto', () => {
    expect(AdminDashboardComponent.colorTipo('AUDIO')).toBe('#FF6B4A');
    expect(AdminDashboardComponent.colorTipo('VIDEO')).toBe('#4361EE');
    expect(AdminDashboardComponent.colorTipo('IMAGEN')).toBe('#2EC4B6');
    expect(AdminDashboardComponent.colorTipo('X')).toBe('#888');
  });

  it('esAdmin() retorna true cuando roles incluye ADMIN', () => {
    expect(AdminDashboardComponent.esAdmin(['USER', 'ADMIN'])).toBeTrue();
  });

  it('esAdmin() retorna false cuando roles no incluye ADMIN', () => {
    expect(AdminDashboardComponent.esAdmin(['USER'])).toBeFalse();
    // JS-W1042: undefined explícito reemplazado por variable tipada
    const rolesVacio: string[] | undefined = undefined;
    expect(AdminDashboardComponent.esAdmin(rolesVacio)).toBeFalse();
  });
  ...
  it('eliminarUsuario() no hace nada si el id no está definido', () => {
    // JS-W1042: undefined explícito reemplazado por variable tipada
    const idVacio: number | undefined = undefined;
    component.eliminarUsuario(idVacio);
    expect(adminService.eliminarUsuario).not.toHaveBeenCalled();
  });

  it('eliminarHistorialDeUsuario() no hace nada si usuarioId no está definido', () => {
    const idVacio: number | undefined = undefined;
    component.eliminarHistorialDeUsuario(idVacio, 'alguien');
