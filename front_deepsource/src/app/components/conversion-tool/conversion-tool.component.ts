...
export class ConversionToolComponent implements OnInit {
  ...
  formatoDestino = '';                // JS-0331
  convirtiendo = false;               // JS-0331
  urlDescarga: string | null = null;
  nombreArchivo = '';                 // JS-0331
  error = '';                         // JS-0331
  exito = false;                      // JS-0331
  ...
  // JS-0105: no usa 'this' de forma indispensable; se mantiene en instancia
  // porque el template lo llama directamente via (dragover)
  ...
  private procesarArchivo(archivo: File): void {
    ...
    if (tamanoMB > this.config.tamanoMaxMB) {
      // JS-0246: concatenación → template literal
  ...
  private validarTipoArchivo(archivo: File): string | null {
    ...
    const extension = (ConversionToolComponent.obtenerExtension(archivo.name) || '').toLowerCase();
    ...
    if (tipoDetectado && tipoDetectado !== this.tipo) {
      const sufijo = tipoDetectado === 'imagen' ? 'imágenes' : `${tipoDetectado}s`;
      // JS-0246: concatenación → template literal
      return `El archivo seleccionado es un ${tipoDetectado}, pero esta sección sólo acepta archivos de ${esperado}. Sube un archivo válido o cambia a la sección de ${sufijo}.`;
    }
  ...
  convertir(): void {
    ...
    this.conversionService.convertir(
      ...
      next: (urlDescarga: string) => {
        ...
        // JS-0339: non-null assertion eliminada → uso de variable local ya validada
        const archivoNombre = this.archivoSeleccionado ? this.archivoSeleccionado.name : '';
        const nombreBase = archivoNombre.replace(/\.[^/.]+$/, '');
  ...
  // JS-0105: no usa 'this' → static
  static obtenerExtension(nombre: string): string | null {
    const idx = nombre.lastIndexOf('.');
    return idx >= 0 ? nombre.substring(idx + 1) : null;
  }

  // Wrappers de instancia para que el template pueda seguir llamándolos
  formatearTamano(bytes: number): string {
    return ConversionToolComponent.formatearTamano(bytes);
  }
}
