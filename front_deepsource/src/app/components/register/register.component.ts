...
export class RegisterComponent {
  ...
  error = '';                 // JS-0331
  mensajeExito = '';          // JS-0331
  mostrarContrasena = false;  // JS-0331

  formCorreo: FormGroup;
  formRegistro: FormGroup;

  correoVerificado = '';      // JS-0331
  ...
  registrar(): void {
    ...
    // JS-C1002: variable 'v' renombrada a 'valores'
  ...
  // JS-0323 + JS-0105: tipado con HttpErrorResponse-compatible interface en lugar de any
  private obtenerTextoErrorRaw(err: unknown): string {
    ...
    if (body && typeof body === 'object') {
      const b = body as { message?: string; trace?: string };
      // JS-0246: concatenación reemplazada por template literal
      return `${b.message ?? ''} ${b.trace ?? ''}`;
    }
